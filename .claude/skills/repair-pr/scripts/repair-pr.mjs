...
// 리뷰 봇 로그인. 이 저장소에는 아직 리뷰 워크플로가 없어서 스레드가 0건으로
// 나온다 — claude-code-review.yml을 붙이면 그대로 동작한다. 다른 봇이 리뷰하는
// 저장소에서 쓸 때만 환경변수로 덮는다.
// 비교는 양쪽에서 `[bot]` 접미사를 벗기므로 REST(claude[bot])와 GraphQL(claude)
// 표기 차이에 걸리지 않는다(authorLoginMatches 참고).
const REVIEW_AUTHOR = process.env.REPAIR_PR_REVIEW_AUTHOR || 'claude[bot]';
...
function replyThreadCommand(rawArgs) {
  ...
  if (options.bodyFile !== null) {
    try {
      ...
      fail(
        `Could not read reply body file ${options.bodyFile}: ${error.message}`,
      );
    }
  }
...
function printReplyThreadHelp() {
  process.stdout
    .write(`Usage: repair-pr.mjs reply-thread <thread-id> (--body <text> | --body-file <path>)
...
function failOnGraphQLErrors(result, operation) {
  if (Array.isArray(result?.errors) && result.errors.length > 0) {
    fail(
      `GraphQL errors while trying to ${operation}: ${JSON.stringify(result.errors)}`,
    );
  }
}
